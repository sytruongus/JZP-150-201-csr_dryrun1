...
%macro t_demog_basechar_mfasfl(trtvar=TRT01A,tab_page_size=15,
   ...
   *** Call JM_AVAL_COUNT macro once for each summary variable ***;                             
   ...
   %jm_aval_count(
      ...
      jm_aval_label=%bquote(Age Group 1, n (%))
	   ...
	   %jm_aval_count(
      jm_indsn=target,jm_var=agegr2n,jm_secondary_where=, jm_fmt=agegr2n., 
      jm_bign=jm_bign1, jm_trtvarn=trtn,jm_block=103, jm_cntvar=usubjid,
      jm_aval_label=%bquote(Age Group 2, n (%))
      );
	  	proc sql;
	  select * from jm_aval_count102;
	quit;
   ...
   %jm_aval_count(
      ...
      jm_bign=jm_bign1, jm_trtvarn=trtn,jm_block=106, jm_cntvar=usubjid,
   ...
   %jm_aval_sum(JM_INDSN=target, jm_var=weightbl, jm_bign=, jm_trtvarn=trtn,    JM_BLOCK=107,JM_SIGD=1 ,
   JM_AVAL_LABEL=%bquote(Weight (kg) at baseline) );

   %jm_aval_sum(JM_INDSN=target, jm_var=Heightbl, jm_bign=, jm_trtvarn=trtn,    JM_BLOCK=108,JM_SIGD=1 ,
