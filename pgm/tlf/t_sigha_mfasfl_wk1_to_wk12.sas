...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS2 ;                             
      where /*TRT01AN in (1,2,3) and  MFASFL='Y' and */ paramcd in ("HAMA115") and .<=avisitn<=12  and anl01fl="Y" and MFASFL="Y"; 
   run;                             
...
length JM_AVAL_LABEL $200. ;
 SET  LSMD_OUT_HL_4 LSMD_OUT_HL_8 LSMD_OUT_HL_12;
 if _name_ ^= ' ' then do ; 
  JM_BLOCK = '106' ;
 ...
 data lsmdiffs_out;
 set  lsmdiffs_out_4 lsmdiffs_out_8 lsmdiffs_out_12;
 run;
