...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS2 ;                             
      where /*TRT01AN in (1,2,3) and  MFASFL='Y' and */ paramcd in ("PHQ0111") and .<=avisitn<=12  and anl01fl="Y" and MFASFL="Y"; 
	  if avisitn=8 then delete;
   run;                             
...
%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE=  ABLFL="Y", JM_AVAL_LABEL=%bquote(Baseline), JM_BYVAR= AVISITN,   JM_BLOCK=101,JM_SIGD=0 );
...
length JM_AVAL_LABEL $200. ;
 SET LSMD_OUT_HL_1 LSMD_OUT_HL_4 LSMD_OUT_HL_12;* LSMD_OUT_HL_12;
 if _name_ ^= ' ' then do ; 
  JM_BLOCK = '106' ;
 ...
 data lsmdiffs_out;
 set lsmdiffs_out_1 lsmdiffs_out_4 lsmdiffs_out_12;* lsmdiffs_out_12;
 run;
