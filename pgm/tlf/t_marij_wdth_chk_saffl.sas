...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS2 ;                             
      where /*TRT01AN in (1,2,3) and  MFASFL='Y' and */ paramcd in ("MWC0123") and avisitn in (0,12,92) and anl01fl="Y" and SAFFL="Y"; 
      trtn=TRTAN;                             
      trt=TRTA; 
      output; 
       if trtn in (1,2 ) ; 
...
%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN = 92, JM_AVAL_LABEL=%bquote(Safety Follow-up),  JM_BLOCK=104,JM_SIGD=0 );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=chg, jm_bign=, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 92, JM_AVAL_LABEL=%bquote(Change from Baseline to Safety Follow-up), JM_BLOCK=105,JM_SIGD=0 );



%JM_AVAL_SUM(JM_INDSN=target, jm_var=chgsfu, jm_bign=, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 92, JM_AVAL_LABEL=%bquote(Change from End of Week 12 to Safety Follow-up), JM_BLOCK=106,JM_SIGD=0 );
...
data jm_aval_allreport1;
 set jm_aval_allreport1;
  ...
  if jm_aval_namec="EMD" then jm_aval_namec="Estimated Median Difference";
   ...
   if jm_block in ("104","105",'106') then pageno=2;
