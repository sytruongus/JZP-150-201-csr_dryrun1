...
%macro t_con_meds_saffl(trtvar=TRT01A,tab_page_size=12,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y' and CONMEDFL='Y')
                       );
   ...
   *** Create TARGET dataset by combing the Working datasets ***;                          
   ...
   table usubjid*trtn/out=frq_sub;
   where SAFFL="Y";
   run;
   ...
   *** Create JM_BIGN(n) by calling JM_BIGN macro. The number of JM_BIGN datasets ***;                               
   ...
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y',JM_VAR=CONMEDFL,                    
      JM_AVAL_LABEL=Number of Participants with at Least One Past Medication,JM_BLOCK=101,JM_TRTVARN=trtn
      );

   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and CMATC4 ne '',
      JM_VAR=CMATC4,                    
      JM_AVAL_LABEL=ATC,JM_BLOCK=102,JM_TRTVARN=trtn
      );
 
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and CMTRT ne '',
   ...
   **Get event counts **;
   %macro gen_evnt_cnt;
      %if &Tab_Event_Count_YN.=Y %then %do;
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y',JM_VAR=CONMEDFL,                    
            JM_AVAL_LABEL=Number of participants cnt,JM_BLOCK=201,JM_TRTVARN=trtn
            );

         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and CMATC4 ne '',
            JM_VAR=CMATC4,                    
            JM_AVAL_LABEL=System organ Class cnt,JM_BLOCK=202,JM_TRTVARN=trtn
            );
 
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and CMTRT ne '',
            JM_VAR=CMTRT, JM_BYVAR=CMATC4,                   
