...
%macro t_ae_soc_pt_spl(trtvar=TRT01A,tab_page_size=16,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y' and TRTEMFL='Y' and AESI="Y")
                       );
   ...
   **Call Pre-processing Macro **;   
   ...
   data _null_;
      ...
      if upcase(evntcnt)='Y' then tab_box='Special Interest Category, n (%) [E]|Preferred Term, n (%) [E]';
      else tab_box='Special Interest Category, n (%)|Preferred Term, n (%)';
   ...
   * APPLY PAGEBREAK ;                                                         
   ...
   data jm_aval_allreport1;                             
    ...
    if JM_BLOCK ne "" and trtn2="" then trtn2="0"; 
   run;                             
