...
%macro t_prior_meds_saffl(trtvar=TRT01A,tab_page_size=14,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y' and PRMEDFL='Y')
                       );
   ...
   *** Create JM_BIGN(n) by calling JM_BIGN macro. The number of JM_BIGN datasets ***;                               
   ...
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y',JM_VAR=PRMEDFL,                    
      JM_AVAL_LABEL=Number of Participants with at Least One Past Medication,JM_BLOCK=101,JM_TRTVARN=trtn
      );

   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y' and CMATC4 ne '',
      JM_VAR=CMATC4,                    
      JM_AVAL_LABEL=ATC,JM_BLOCK=102,JM_TRTVARN=trtn
      );
 
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y' and CMTRT ne '',
   ...
   **Get event counts **;
   %macro gen_evnt_cnt;
      %if &Tab_Event_Count_YN.=Y %then %do;
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y',JM_VAR=PRMEDFL,                    
            JM_AVAL_LABEL=Number of participants cnt,JM_BLOCK=201,JM_TRTVARN=trtn
            );

         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y' and CMATC4 ne '',
            JM_VAR=CMATC4,                    
            JM_AVAL_LABEL=System organ Class cnt,JM_BLOCK=202,JM_TRTVARN=trtn
            );
 
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=PRMEDFL='Y' and CMTRT ne '',
            JM_VAR=CMTRT, JM_BYVAR=CMATC4,                   
