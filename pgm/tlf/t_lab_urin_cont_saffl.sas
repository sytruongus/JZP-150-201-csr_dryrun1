...
data ADLB;
 set adam.ADLB ;
 if SAFFL="Y" and  avisitn in (0,1,4,8,12) and parcat1="Urinalysis" and ANL01FL='Y' and anl05fl='Y' and AVAL ne . and LBSTAT ne "NOT DONE";
...
/*%JM_AVAL_COUNT(JM_INDSN=ADLB_F, jm_var= AVALC , JM_SECONDARY_WHERE= avalc ne "" and aval eq . and  paramn  in (106), jm_bign=jm_bign1, jm_trtvarn=trtn,*/
/*jm_block=112, jm_cntvar=usubjid, JM_BYVAR= paramn param avisitn avisit AVALN, JM_FMT=, JM_AVAL_LABEL=%bquote(SNRARRY) );*/
/**/
/*%JM_AVAL_COUNT(JM_INDSN=ADLB_F, jm_var= AVALC , JM_SECONDARY_WHERE= avalc ne "" and aval eq . and  paramn  in (201), jm_bign=jm_bign1, jm_trtvarn=trtn,*/
...
%JM_PGBRK (JM_INDSN1=JM_AVAL_TRANS1, JM_INDSN2=, JM_BREAKCNT=12, JM_CONTOPT=N, JM_GROUPOPT=N, JM_OUTDSN=JM_AVAL_ALLREPORT1);
