...
%stats_b(N=1,SUBGRP_LAB=%str(Gender : Male ));

%stats_b(N=2,SUBGRP_LAB=%str(Gender : Female));
...
data dbrw_data;
      set ADQS ;
      if chg ne . and trtn in (1,2,3) and SEXN ne . ;
    	id=subjid;
...
class id avisitn  trtn SSNRISN SEXN  wk12;
model chg=base trtn wk12  SSNRISN  base*wk12 SEXN*trtn trtn*wk12 SEXN*wk12 SEXN*trtn*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
lsmeans trtn*SEXN/ cl alpha=0.05 ;
      ...
      if chg ne . and trtn in (1,2,3) and SSNRISN ne . ;
		...
		if avisitn=12 then wk12=1 ; 
	        ...
	        if SSNRISN=1 then SSNRISN1=1;
	   	    ...
	   	    if SEXN=1 then SEXN1=1;
	   else if SEXN=2 then SEXN1=0; 
 run;
...
model chg=base trtn1 wk12 SSNRISN1 SEXN1 base*wk12 SEXN1*trtn1 trtn1*wk12 SEXN1*wk12 SEXN1*trtn1*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO  Week 4" trtn1 1 SEXN1*trtn1 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn1 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1 SEXN1*trtn1 1 SEXN1*trtn1*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" SEXN1*trtn1 1;
contrast "Treatment*Week 12*Subgroup Interaction" SEXN1*trtn1 1 SEXN1*trtn1*wk12 1;
...
where trtn in (2,3);
class id avisitn SSNRISN1 ;
model chg=base trtn2 wk12 SSNRISN1 SEXN1 base*wk12 SEXN1*trtn2 trtn2*wk12 SEXN1*wk12 SEXN1*trtn2*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO    Week 4" trtn2 1 SEXN1*trtn2 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn2 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1 SEXN1*trtn2 1 SEXN1*trtn2*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" SEXN1*trtn2 1;
contrast "Treatment*Week 12*Subgroup Interaction" SEXN1*trtn2 1 SEXN1*trtn2*wk12 1;
...
set LSM_est1 LSM_est2;
     ...
     if label="1 ACT-PBO  Week 12" then SEXN=1;
else if label="2 ACT-PBO  Week 12" then SEXN=2;
...
JM_BLOCK1=strip(JM_BLOCK)||strip(put(SEXN,best.));
JM_BLOCK=JM_BLOCK1;
run;
...
%Macro stats_null(n=,SUBGRP_LAB=);
 ...
 Data final&n ;
 length jm_aval_label SEX _name_ _label_ trtn1 trtn2 $200 ;
  set JM_AVAL_TRANS&n(in=a) all_mixed_stat(in=b where=(SEXN=&n));
...
%mend;
%stats_null(n=1,SUBGRP_LAB=%str(Gender : Male ));



%stats_null(n=2,SUBGRP_LAB=%str(Gender : Female));
...
OPTIONS FORMCHAR='|_---|+|---+=|-/\<>*'; 
title%eval(&lasttitlenum.+1) j=l "Randomization stratum:  #BYVAL(SEX)";

   %JM_AVAL_REPORT (JM_INDSN=jm_aval_allreport&myrep.,JM_BYVAR= SEX, JM_BIGNDSN=Jm_bign1, 
