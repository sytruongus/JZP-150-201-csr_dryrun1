...
%macro t_pst_surg_medcon_mfasfl(trtvar=TRT01A,tab_page_size=11,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y' and index(MHCAT,"MEDICAL HISTORY")>0  and mhterm ^= "PTSD")
                       );
   ...
   *** Create JM_BIGN(n) by calling JM_BIGN macro. The number of JM_BIGN datasets ***;                               
   ...
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=,JM_VAR=MHCAT,                    
      JM_AVAL_LABEL=%str(Number of Participants with at Least One Past Surgical or Medical Conditions/Diseases, Other than PTSD) ,JM_BLOCK=101,JM_TRTVARN=trtn
      );

   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=MHBODSYS ne '',
      JM_VAR=MHBODSYS,                    
      JM_AVAL_LABEL=System organ Class,JM_BLOCK=102,JM_TRTVARN=trtn
      );
 
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=MHDECOD ne '',
      JM_VAR=MHDECOD, JM_BYVAR=MHBODSYS,                   
   ...
   **Get event counts **;
   %macro gen_evnt_cnt;
      %if &Tab_Event_Count_YN.=Y %then %do;
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=,JM_VAR=MHCAT,                    
            JM_AVAL_LABEL=Number of participants cnt,JM_BLOCK=201,JM_TRTVARN=trtn
            );

         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=MHBODSYS ne '',
            JM_VAR=MHBODSYS,                    
            JM_AVAL_LABEL=System organ Class cnt,JM_BLOCK=202,JM_TRTVARN=trtn
            );
 
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=MHDECOD ne '',
            JM_VAR=MHDECOD, JM_BYVAR=MHBODSYS,                   
   ...
   * Format the dataset for Indentation and sorting ;                                                       
   ...
   data JM_AVAL_TRANS2;                        
      ...
      if grp_ord='103' and trtn2 not in('','0') then mhdecod_lin= input(scan(trtn2,1,'('),best.);                        
      else if grp_ord='103' and trtn2 in('0') then mhdecod_lin=0;                        
      else mhdecod_lin=999999;                        
   run;                        

   proc sort data=JM_AVAL_TRANS2;                        
      by jm_block jm_aval_label block_num grpvar descending mhdecod_lin rec_num;                        
   run; 

   *** Get Total subjects per AEDECOD to be used for sorting if needed ***;  
   %macro sort;                           
      proc sql;                             
         create table socptmax_pt_ord as select distinct jm_block,block_num,jm_aval_label,jm_aval_namec,sum(mhdecod_lin) as sum_lin from jm_aval_trans2 
   ...
   data jm_aval_trans2x;                             
      ...
      if mhdecod_lin ne 999999 and sum_lin ne . and block_num=3 then mhdecod_lin=sum_lin;
   run;                             
   ...
   * APPLY PAGEBREAK ;                                                         
   ...
   proc sort data=jm_aval_allreport1;                             
      by pageno jm_block jm_aval_label block_num descending mhdecod_lin rec_num jm_aval_namen;                             
   run;                             
