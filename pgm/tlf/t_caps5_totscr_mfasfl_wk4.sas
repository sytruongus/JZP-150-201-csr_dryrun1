...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS ;                             
      where /*TRT01AN in (1,2,3) and  MFASFL='Y' and */ paramcd in ("PTSDTSEV") and .<=avisitn<=4  and anl01fl="Y" and MFASFL="Y"; 
   run;                             
...
%macro LSM;
	...
	** shift location, H-L Estimate **;
		...
		*p-value for estimated median;
		 ...
		 proc mixed data=ranked method=reml;
            class id avisitn SSNRISN  trtn;
            model rchg=rbase trtn avisitn trtn*avisitn rbase*avisitn SSNRISN SSNRISN*avisitn;
            repeated avisitn/type=un sub=id;
            lsmeans trtn*avisitn / diff cl alpha=0.05;
            ods output LSMeans=LSM_temp_hl  diffs=ls_diffs_hl ;
         run;

data Manova;
set ls_diffs_hl;
   by trtn ;
   where avisitn=4  and trtn in (1,2) and _trtn=3;
...
*ci for estimated median;
	 ...
	 where trtn in (2,3);
		...
		merge HLestimate(in=a)
		      MANOVA(in=b keep=trtn Probt);
		by trtn;
		keep trtn shift lowerCL upperCL Probt;
		format shift 9.2;
	   run;
...
length JM_AVAL_LABEL $200. ;
 ...
 length jm_aval_label _name_ _label_ $200 ;
  set JM_AVAL_TRANS1 LSM_STAT lsmdiffs_out LSMD_OUT_HL ;
  if jm_aval_label = 'Change' then do;
    jm_aval_label = "Change from Baseline to End of Week 4" ;
    jm_block="103";
  end;
  ...
  if jm_aval_namec="EMD" then jm_aval_namec="Estimated Median Difference";
   ...
   if jm_block = ' ' then delete ;
	  ...
	  jm_aval_namen=orig_rec_num;
	    ...
	    if jm_block in ("101","102") then pageno=1;
       if jm_block in ("103") and groupvarn<=18 then pageno=2;
	   else if jm_block in ("103") and groupvarn>18 then pageno=3;
