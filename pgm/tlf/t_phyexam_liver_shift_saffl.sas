...
%macro t_phyexam_liver_shift_saffl( TRTVAR=, ADSL_SUBSET=%str(trt01aN in (1,2,3) and saffl='Y'), ADLB_SUBSET=%STR(trtaN in (1,2,3) and ANL01FL='Y'),
   ...
   **Read ADSL dataset and subset as needed**;
   ...
   data adpe;
   set adpe;
   if basec="Abnormal - Not Clinically Significant" then basec="ABNORMALNCS";
   if basec="Abnormal - Clinically Significant" then basec="ABNORMALCS";

   if AVALC="Abnormal - Not Clinically Significant" then AVALC="ABNORMALNCS";
   if AVALC="Abnormal - Clinically Significant" then AVALC="ABNORMALCS";
   ...
   **Get Post Baseline data and subset as needed**;
   proc sort data=adpe out=adlbpost(keep=usubjid &trtvar.: avisitn param: &POSTCATVAR.: &BASECATVAR.) nodupkey;
...
/*   run;*/
   ...
   **Get Baseline data**;
   proc sort data=adpe out=adlbbase(keep=usubjid &trtvar.: param: &POSTCATVAR. rename=(&POSTCATVAR.=&BASECATVAR.)) nodupkey;
      by usubjid paramcd avisitn &POSTCATVAR.;
   ...
   **Create numeric variables for &POSTCATVAR./&BASECATVAR.**;
   data adlb1_;
      ...
      if &POSTCATVAR. eq '' then
         &POSTCATVAR.='Missing';

      if &BASECATVAR. eq '' then
         &BASECATVAR.='Missing';

      if &POSTCATVAR. ne '' then
         &POSTCATVAR.n=input(upcase(&POSTCATVAR.),inlbshift.);

      if &BASECATVAR. ne '' then
         &BASECATVAR.n=input(upcase(&BASECATVAR.),inlbshift.);
      length shiftcat $100;
...
data alldummycat;
   ...
   do i=&shift_list.;
      &POSTCATVAR.n=i;
      jm_aval_namec=strip(put(&POSTCATVAR.n,peshift.));
      output;
   end;
...
%JM_PGBRK (JM_INDSN1=JM_AVAL_TRANS4, JM_BREAKCNT=14, JM_CONTOPT=N, JM_GROUPOPT=Y, JM_OUTDSN=JM_AVAL_ALLREPORT1);
...
data jm_bign1;
   ...
   do i=&shift_list.;
      ...
      TRTN=I;
      JM_AVAL_BIGN=0;
      JM_AVAL_BIGN_LABEL=strip(scan(PUT(i,peshift.),2,':'));
...
data jm_aval_allreport2;
   ...
   if lowcase(strip(jm_aval_namec))='missing' then delete;
run;
   ...
   %do myrep =1 %to &n_all_blocks.;
      ...
      *-----------------------------------------------------------------;
      ...
      data jm_aval_allreport;
		 ...
		 if JM_aval_namec="^{nbspace 2}Total" then JM_aval_namec="^{nbspace 2}Total^n";

		 drop trtn99;
      run;

      %JM_AVAL_REPORT (JM_INDSN=Jm_aval_allreport,JM_BYVAR= , JM_BIGNDSN=Jm_bign1, jm_spanheadopt=Y , 
         JM_INDENTOPT=N, jm_breakopt=N, jm_breakvar=JM_AVAL_LABEL,JM_TRTWIDTH=0.8in);
   %end;
...
data tlfdata.&OUTPUTdt.;
   set jm_aval_allreport2(drop=trtn99);
   keep paramcd param paramn trt: jm_trtvarn avisit: jm_block jm_aval_label jm_aval_namec &POSTCATVAR.n;
  
run;

%mend t_phyexam_liver_shift_saffl;

%t_phyexam_liver_shift_saffl(TRTVAR=TRTA, postbase_avisitn=%str(4,8,12), base_avisitn=0,
   ADLB_SUBSET=%STR(&TRTVAR.N in (1,2,3) and saffl='Y' and paramn=101 and ANL01FL='Y' ),
   BASECATVAR=BASEC,POSTCATVAR=AVALC,
   tab_page_size=12,drop_missing_row=N,drop_missing_column=N,drop_zero_rows=N,BIGN_DATA=ADLB);
