...
%macro t_ae_soc_pt_2pct(trtvar=TRT01A,tab_page_size=14,Tab_Event_Count_YN=N,tab_sort=FREQ,
 ...
 /******************Arun Added below conditions for >=2 % *****/ 
      *rec_num=_n_;     
      pct_chk1= tranwrd(scan(trtn99,2,"("),")","");
      pct_chk= input(pct_chk1,best.);  
   ...
   data JM_AVAL_TRANS2;                        
      ...
      if grp_ord='103' and trtn99 not in('','0') then aedecod_lin= input(scan(trtn99,1,'('),best.);                        
      else if grp_ord='103' and trtn99 in('0') then aedecod_lin=0;                        
   ...
   *REPORT- PROC REPORT MODULE ;                                                         
   %JM_AVAL_REPORT(
      ...
      JM_INDENTOPT=N,JM_GROUPLABEL=,JM_CELLWIDTH=3.0in,JM_TRTWIDTH=,JM_SPANHEADOPT=Y,JM_BYVAR=,JM_REPTYPE=Table);                   
