...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS ;                             
      where paramcd in ("CA5BTOSV","CA5CTOSV","CA5DTOSV","CA5ETOSV") and .<=avisitn<=12  and anl01fl="Y" and MFASFL="Y"; 
                                                                                          
	  if paramcd in ("CA5BTOSV") then PARAMN=1;
	  if paramcd in ("CA5CTOSV") then PARAMN=2;
      if paramcd in ("CA5ETOSV") then PARAMN=3;
      if paramcd in ("CA5DTOSV") then PARAMN=4;
   ...
   table parcat1*parcat2*PARAMN*paramcd*param/out=freq_param nocol nopct;
   run;
...
%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN=0 and ABLFL="Y", JM_AVAL_LABEL=%bquote(Baseline),   JM_BLOCK=101,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit);

%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN = 1 , JM_AVAL_LABEL=%bquote(End of Week 1),  JM_BLOCK=102,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=chg, jm_bign=, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 1   , JM_AVAL_LABEL=%bquote(Change from Baseline to End of Week 1), JM_BLOCK=103,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN = 4 , JM_AVAL_LABEL=%bquote(End of Week 4),  JM_BLOCK=104,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=chg, jm_bign=, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 4   , JM_AVAL_LABEL=%bquote(Change from Baseline to End of Week 4), JM_BLOCK=105,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=aval, jm_bign=, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN = 12 , JM_AVAL_LABEL=%bquote(End of Week 12),  JM_BLOCK=106,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );

%JM_AVAL_SUM(JM_INDSN=target, jm_var=chg, jm_bign=, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 12   , JM_AVAL_LABEL=%bquote(Change from Baseline to End of Week 12), JM_BLOCK=107,JM_SIGD=0, JM_BYVAR=PARAMN paramcd param avisitn avisit );
...
%JM_AVAL_SUM_TRANS(JM_AVAL_INPUT=Jm_aval_alldata1(where=(JM_TYPE="SUMMARY")), 
 ...
 by PARAMN paramcd param avisitn avisit JM_BLOCK ord;
 run;
...
by paramn paramcd param avisitn trtn;
run;
...
*ci for estimated median;
	 ...
	 where trtn in (1,3);
		  ...
		  by paramn paramcd param avisitn;
        run;	
	 ...
	 where trtn in (2,3);
		...
		merge HLestimate_&ds(in=a)
		      MANOVA_&ds(in=b keep=paramn paramcd param avisitn trtn Probt);
		by paramn paramcd param avisitn trtn;
		keep paramn paramcd param avisitn trtn shift lowerCL upperCL Probt;
		format shift 9.2;
	   run;
...
length JM_AVAL_LABEL $200. ;
 SET LSMD_OUT_HL_4 LSMD_OUT_HL_12;
 if _name_ ^= ' ' then do ; 
  JM_BLOCK = '106' ;
 ...
 keep paramn paramcd param avisitn jm_block JM_AVAL_LABEL JM_AVAL_NAMEC _NAME_ _LABEL_ trtn1 trtn2 ord;
 RUN;
 data lsmdiffs_out;
 set  lsmdiffs_out_4  lsmdiffs_out_12;
 run;
...
by paramn paramcd param avisitn jm_blockn ord;
run;
...
data jm_aval_allreport1;
 set jm_aval_allreport1;
  ...
  if jm_aval_namec="EMD" then jm_aval_namec="Estimated Median Difference";
   ...
   if jm_aval_namec="p-value" and ord=13 then jm_aval_namec="p-value^n";
   ...
   if paramn=1 then do;
	   if jm_block in ("101") then pageno=1;
		 if jm_block in ("102") then pageno=2;
            if jm_block in ("103") and ord<=5 then pageno=4;
	   else if jm_block in ("103") and ord>5 then pageno=5;

	    if jm_block in ("104") then pageno=6;
            if jm_block in ("105") and ord<=5 then pageno=7;
	   else if jm_block in ("105") and ord>5  then pageno=8;
            if jm_block in ("106") then pageno=9;
            if jm_block in ("107") and ord<=5 then pageno=10;
	   else if jm_block in ("107") and ord>5  then pageno=11;

            if jm_block in ("108") then pageno=12;
            if jm_block in ("109") and ord<=5 then pageno=13;
	   else if jm_block in ("109") and ord>5  then pageno=14;
	end;

if paramn=2 then do;
            if jm_block in ("101") then pageno=15;
		    if jm_block in ("102") then pageno=16;
            if jm_block in ("103") and ord<=5 then pageno=17;
	   else if jm_block in ("103") and ord>5 then pageno=18;

	    if jm_block in ("104") then pageno=19;
            if jm_block in ("105") and ord<=5 then pageno=20;
	   else if jm_block in ("105") and ord>5  then pageno=21;
    
	    if jm_block in ("106") then pageno=22;
            if jm_block in ("107") and ord<=5 then pageno=23;
	   else if jm_block in ("107") and ord>5  then pageno=24;

            if jm_block in ("108") then pageno=25;
            if jm_block in ("109") and ord<=5 then pageno=26;
	   else if jm_block in ("109") and ord>5  then pageno=27;
	end; 


	if paramn=3 then do;
            if jm_block in ("101") then pageno=28;
		    if jm_block in ("102") then pageno=29;
            if jm_block in ("103") and ord<=5 then pageno=30;
	   else if jm_block in ("103") and ord>5 then pageno=31;

	    if jm_block in ("104") then pageno=32;
            if jm_block in ("105") and ord<=5 then pageno=33;
	   else if jm_block in ("105") and ord>5  then pageno=34;
    
	    if jm_block in ("106") then pageno=35;
            if jm_block in ("107") and ord<=5 then pageno=36;
	   else if jm_block in ("107") and ord>5  then pageno=37;

            if jm_block in ("108") then pageno=38;
            if jm_block in ("109") and ord<=5 then pageno=39;
	   else if jm_block in ("109") and ord>5  then pageno=40;
	end; 


	if paramn=4 then do;
            if jm_block in ("101") then pageno=41;
		    if jm_block in ("102") then pageno=42;
            if jm_block in ("103") and ord<=5 then pageno=43;
	   else if jm_block in ("103") and ord>5 then pageno=44;

	    if jm_block in ("104") then pageno=45;
            if jm_block in ("105") and ord<=5 then pageno=46;
	   else if jm_block in ("105") and ord>5  then pageno=47;
    
	    if jm_block in ("106") then pageno=48;
            if jm_block in ("107") and ord<=5 then pageno=49;
	   else if jm_block in ("107") and ord>5  then pageno=50;

            if jm_block in ("108") then pageno=51;
            if jm_block in ("109") and ord<=5 then pageno=52;
	   else if jm_block in ("109") and ord>5  then pageno=53;
	end; 
