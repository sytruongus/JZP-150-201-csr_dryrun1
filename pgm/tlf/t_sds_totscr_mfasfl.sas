...
options orientation=landscape missing=' ' nodate nonumber;
   ...
   *** Create a macro variable for storing ADQS dataset name from the list of datasets ***;                             
   data ADQS ;                             
      set adam.ADQS2 ;                             
      where /*TRT01AN in (1,2,3) and  MFASFL='Y' and */ paramcd in ("SDS0106") and .<=avisitn<=12  and anl01fl="Y" and MFASFL="Y"; 
   run;                             
...
proc transpose data=LSM_data out=lsmdata_stat;
id trtn;
...
data jm_aval_allreport1;
 ...
 if jm_aval_namec="Min, Max" and ord=5 then jm_aval_namec="Min, Max^n";
if jm_aval_namec="LS Mean Diff" and ord=10 then jm_aval_namec="LS Mean Diff to Placebo";
   ...
   if jm_block = ' ' then delete ;
	  ...
	  jm_aval_namen=orig_rec_num;
       ...
       if jm_block in ("103") and groupvarn<=19 then pageno=2;
	   else if jm_block in ("103") and groupvarn>19 then pageno=3;
