...
%macro t_pst_surg_medcon_saffl(trtvar=TRT01A,tab_page_size=11,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y' and index(MHCAT,"MEDICAL HISTORY")>0 and mhterm ^= "PTSD" )
                       );
