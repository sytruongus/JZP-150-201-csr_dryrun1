 ...
 *            ADSL_SUBSET     - ADSL subset condition to be used. ex: &TRTVAR.N in (1,2,3,4) and MFASFL='Y'
 *            ADAE_SUBSET     - ADAE subset condition to be used. ex: &TRTVAR.N in (1,2,3,4) and MFASFL='Y' and 
 *                                  CONMEDFL='Y' and PLBFL ne 'Y'
 ************************************************************************
...
%macro t_con_meds_disrel_mfasfl(trtvar=TRT01A,tab_page_size=14,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y' and CONMEDFL='Y' and DISRELFL="Y")
                       );
   ...
   **Call Pre-processing Macro **;   
   ...
   data _null_;
      ...
      if upcase(evntcnt)='Y' then tab_box='Anatomical Therapeutic Class Level 4, n (%) [E]|Preferred Drug Name, n (%) [E]';
      else tab_box='Anatomical Therapeutic Class Level 4, n (%)|Preferred Drug Name, n (%)';
   ...
   *** Create JM_BIGN(n) by calling JM_BIGN macro. The number of JM_BIGN datasets ***;                               
   %JM_BIGN(JM_INDSN=target,JM_CNTVAR=usubjid,JM_TRTVARN=trtn,JM_TRTFMT=trt,JM_BIGN_WHERE=%bquote(MFASFL='Y'),JM_SUFFIX=1);                   

   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y'and DISRELFL="Y",JM_VAR=CONMEDFL,                    
      JM_AVAL_LABEL=Number of Participants with at Least One Past Medication,JM_BLOCK=101,JM_TRTVARN=trtn
      );
   ...
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and CMATC4 ne '' and DISRELFL="Y",
      JM_VAR=CMATC4,                    
      JM_AVAL_LABEL=ATC,JM_BLOCK=102,JM_TRTVARN=trtn
      );
 
   %JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and DISRELFL="Y" and CMTRT ne '',
      JM_VAR=CMTRT, JM_BYVAR=CMATC4,                   
      JM_AVAL_LABEL=Preferred Drun Name,JM_BLOCK=103,JM_TRTVARN=trtn
      );
   ...
   **Get event counts **;
   %macro gen_evnt_cnt;
      %if &Tab_Event_Count_YN.=Y %then %do;
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and DISRELFL="Y",JM_VAR=CONMEDFL,                    
            JM_AVAL_LABEL=Number of participants cnt,JM_BLOCK=201,JM_TRTVARN=trtn
            );

         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and DISRELFL="Y" and CMATC4 ne '',
            JM_VAR=CMATC4,                    
            JM_AVAL_LABEL=System organ Class cnt,JM_BLOCK=202,JM_TRTVARN=trtn
            );
 
         %JM_AVAL_COUNT(
            JM_INDSN=target,JM_CNTVAR=evnt,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=CONMEDFL='Y' and DISRELFL="Y" and CMTRT ne '',
            JM_VAR=CMTRT, JM_BYVAR=CMATC4,                   
   ...
   * Format the dataset for Indentation and sorting ;                                                       
   ...
   data JM_AVAL_TRANS2;                        
      ...
      if grp_ord='103' and trtn2 not in('','0') then CMTRT_lin= input(scan(trtn2,1,'('),best.);                        
      else if grp_ord='103' and trtn2 in('0') then CMTRT_lin=0;                        
      else CMTRT_lin=999999;                        
   run;                        

   proc sort data=JM_AVAL_TRANS2;                        
      by jm_block jm_aval_label block_num grpvar descending CMTRT_lin rec_num;                        
   run; 

   *** Get Total subjects per CMTRT to be used for sorting if needed ***;  
   %macro sort;                           
      proc sql;                             
         create table socptmax_pt_ord as select distinct jm_block,block_num,jm_aval_label,jm_aval_namec,sum(CMTRT_lin) as sum_lin from jm_aval_trans2 
   ...
   data jm_aval_trans2x;                             
      ...
      if CMTRT_lin ne 999999 and sum_lin ne . and block_num=3 then CMTRT_lin=sum_lin;
   run;                             
   ...
   * APPLY PAGEBREAK ;                                                         
   ...
   proc sort data=jm_aval_allreport1;                             
      by pageno jm_block jm_aval_label block_num descending CMTRT_lin rec_num jm_aval_namen;                             
   run;                             
