...
%macro t_ae_soc_pt_sev(trtvar=TRT01A,tab_page_size=12,Tab_Event_Count_YN=N,tab_sort=FREQ,
                           analysis_var=ASEVN,analysis_var_fmt=aesev,
   ...
   **Call Pre-processing Macro **; 
   ...
   data _null_;
      ...
      if upcase(evntcnt)='Y' then tab_box='System Organ Class|Severity, n (%) [E]|  Preferred Term|    Severity, n (%) [E]';
      else tab_box='System Organ Class|Severity, n (%)|  Preferred Term|    Severity, n (%)';
   ...
   *** For ADAE reports by worst/max sevrity/toxicity/relationship/outcome create flag variables for 
   ...
   proc sql;                             
      ...
      &analysis_var._subj 
      from adae as a left join (select distinct usubjid,trtn, max(&analysis_var.) as &analysis_var._subj from ADAE 
      ...
      &analysis_var._soc 
      from ADAE__1 as a left join (select distinct usubjid,trtn,aebodsys, max(&analysis_var.) as &analysis_var._soc 
      from ADAE__1 
