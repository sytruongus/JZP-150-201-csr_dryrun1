...
%stats_b(N=1,SUBGRP_LAB=%str(Age Group 1 : <= 65 ));

%stats_b(N=2,SUBGRP_LAB=%str(Age Group 1  : > 65));
...
class id avisitn  trtn SSNRISN AGEGR1N  wk12;
model chg=base trtn wk12  SSNRISN  base*wk12 AGEGR1N*trtn trtn*wk12 AGEGR1N*wk12 AGEGR1N*trtn*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
lsmeans trtn*AGEGR1N/ cl alpha=0.05 ;
      ...
      if chg ne . and trtn in (1,2,3) and SSNRISN ne . ;
		...
		if avisitn=12 then wk12=1 ; 
	        ...
	        if SSNRISN=1 then SSNRISN1=1;
	   	    ...
	   	    if AGEGR1N=1 then AGEGR1N1=1;
	   else if AGEGR1N=2 then AGEGR1N1=0; 
 run;
...
model chg=base trtn1 wk12 SSNRISN1 AGEGR1N1 base*wk12 AGEGR1N1*trtn1 trtn1*wk12 AGEGR1N1*wk12 AGEGR1N1*trtn1*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO  Week 4" trtn1 1 AGEGR1N1*trtn1 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn1 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1 AGEGR1N1*trtn1 1 AGEGR1N1*trtn1*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" AGEGR1N1*trtn1 1;
contrast "Treatment*Week 12*Subgroup Interaction" AGEGR1N1*trtn1 1 AGEGR1N1*trtn1*wk12 1;
...
where trtn in (2,3);
class id avisitn SSNRISN1 ;
model chg=base trtn2 wk12 SSNRISN1 AGEGR1N1 base*wk12 AGEGR1N1*trtn2 trtn2*wk12 AGEGR1N1*wk12 AGEGR1N1*trtn2*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO    Week 4" trtn2 1 AGEGR1N1*trtn2 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn2 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1 AGEGR1N1*trtn2 1 AGEGR1N1*trtn2*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" AGEGR1N1*trtn2 1;
contrast "Treatment*Week 12*Subgroup Interaction" AGEGR1N1*trtn2 1 AGEGR1N1*trtn2*wk12 1;
...
set LSM_est1 LSM_est2;
     ...
     if label="1 ACT-PBO  Week 12" then AGEGR1N=1;
else if label="2 ACT-PBO  Week 12" then AGEGR1N=2;
...
JM_BLOCK1=strip(JM_BLOCK)||strip(put(AGEGR1N,best.));
JM_BLOCK=JM_BLOCK1;
run;
...
%Macro stats_null(n=,SUBGRP_LAB=);
 ...
 Data final&n ;
 length jm_aval_label AGEGR1 _name_ _label_ trtn1 trtn2 $200 ;
  set JM_AVAL_TRANS&n(in=a) all_mixed_stat(in=b where=(AGEGR1N=&n));
...
%mend;
%stats_null(n=1,SUBGRP_LAB=%str(Age Group 1 : <= 65 ));
%stats_null(n=2,SUBGRP_LAB=%str(Age Group 1 : > 65 ));
...
OPTIONS FORMCHAR='|_---|+|---+=|-/\<>*'; 
title%eval(&lasttitlenum.+1) j=l "Randomization stratum:  #BYVAL(AGEGR1)";

   %JM_AVAL_REPORT (JM_INDSN=jm_aval_allreport&myrep.,JM_BYVAR= AGEGR1, JM_BIGNDSN=Jm_bign1, 
...
%let dsname=T_9_02_01_03_02;
data tlfdata.&dsname;
length  TRTN1-TRTN3 $200.;
set final_subgrp_:;

keep JM_: TRTN: AGEGR1:;
run;
