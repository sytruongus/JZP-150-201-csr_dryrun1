...
%macro t_ae_soc_pt_rel(trtvar=TRT01A,tab_page_size=16,Tab_Event_Count_YN=N,tab_sort=FREQ,
                       adsl_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y'),
                       adae_subset=%str(&TRTVAR.N in (1,2,3) and SAFFL='Y' and TRTEMFL='Y' and AEDISCON="Y")
                       );
   ...
   * Format the dataset for Indentation and sorting ;                                                       
   ...
   proc sql noprinit;
     select distinct "trtn"||strip(put(jm_trtvarn,best.)), count(distinct jm_trtvarn) into: bign_Trtlist separated by ' ', : bign_ntrt
     from jm_bign1;
   quit;
   ...
   data JM_AVAL_TRANS2;                        
      ...
      by block_num;                        
	   %do i=1 %to &bign_ntrt.;
			if %scan(&bign_trtlist.,&i.," ") eq '' then %scan(&bign_trtlist.,&i.," ")='0';
	   %end;
