...
%macro t_ptsd_mfasfl(trtvar=TRT01A,tab_page_size=16,
                   adsl_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y'),
				   adexsum_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y' and upcase(mhcat)="POSTTRAUMATIC STRESS DISORDER HISTORY")
                 );
 ...
 proc sort data=target out=admh_ptsd;
   ...
   *** Call JM_AVAL_COUNT macro once for each summary variable ***;                             
   ...
   JM_AVAL_LABEL=%bquote(Time since diagnosis(years)) );
	...
	%JM_AVAL_COUNT(
      JM_INDSN=target,JM_CNTVAR=usubjid,JM_BIGN=jm_bign1,JM_SECONDARY_WHERE=,JM_VAR=MHENRTPT,                    
