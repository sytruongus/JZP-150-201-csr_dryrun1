...
DATA ADQS ;
 SET adam.ADQS ;
  ...
  where /*TRT01AN in (1,2,3) and*/  MFASFL='Y' and paramcd in ("PTSDTSEV") and .<avisitn<=12 and ANL01FL="Y"; 
...
%JM_BIGN(JM_INDSN=adsl&n,JM_CNTVAR=usubjid,jm_suffix=0, jm_trtvarn=trtn, jm_trtfmt=trt );
DATA JM_BIGN&N.;SET JM_BIGN0;RUN;

*----------------------------------------------------------------------------------------------------------------------------------;
* CALL SUMMARYSTAT FOR DESCRIPTIVE AND COUNTF FOR THE CATEGORICAL IN THE ORDER OF TABLE SHELL AND INCREMENT BLOCK NUMBERS;
*----------------------------------------------------------------------------------------------------------------------------------;

%JM_AVAL_SUM(JM_INDSN=adqs&N, jm_var=aval,  jm_bign=jm_bign0, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN=0 and ABLFL="Y", JM_AVAL_LABEL=%bquote(Baseline),   JM_BLOCK=101&N,JM_SIGD=0 );

%JM_AVAL_SUM(JM_INDSN=adqs&N, jm_var=aval,  jm_bign=jm_bign0, jm_trtvarn=trtn, JM_SECONDARY_WHERE= AVISITN = 12 , JM_AVAL_LABEL=%bquote(End of Week 12),  JM_BLOCK=102&N,JM_SIGD=0 );

%JM_AVAL_SUM(JM_INDSN=adqs&N, jm_var=chg,   jm_bign=jm_bign0, jm_trtvarn=trtn,  JM_SECONDARY_WHERE= AVISITN = 12   , JM_AVAL_LABEL=%bquote(Change from Baseline to End of Week 12), JM_BLOCK=103&N,JM_SIGD=0 );
...
%stats_b(N=1,SUBGRP_LAB=%str(VISIT : On-site ));

%stats_b(N=2,SUBGRP_LAB=%str(VISIT : Remote));
...
data dbrw_data;
      set ADQS ;
      if chg ne . and trtn in (1,2,3) and VISITTYN ne . ;
    	id=subjid;
...
class id avisitn  trtn SSNRISN VISITTYN  wk12;
model chg=base trtn wk12  SSNRISN  base*wk12 VISITTYN*trtn trtn*wk12 VISITTYN*wk12 VISITTYN*trtn*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
lsmeans trtn*VISITTYN/ cl alpha=0.05 ;
      ...
      if chg ne . and trtn in (1,2,3) and SSNRISN ne . ;
		...
		if avisitn=12 then wk12=1 ; 
	        ...
	        if SSNRISN=1 then SSNRISN1=1;
	   	    ...
	   	    if VISITTYN=1 then VISITTYN1=1;
	   else if VISITTYN=2 then VISITTYN1=0; 
 run;
...
proc mixed data=dbrw_data;
where trtn in (1,3);
class id avisitn SSNRISN1 ;
model chg=base trtn1 wk12 SSNRISN1 VISITTYN1 base*wk12 VISITTYN1*trtn1 trtn1*wk12 VISITTYN1*wk12 VISITTYN1*trtn1*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO  Week 4" trtn1 1 VISITTYN1*trtn1 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn1 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1 VISITTYN1*trtn1 1 VISITTYN1*trtn1*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn1 1 trtn1*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" VISITTYN1*trtn1 1;
contrast "Treatment*Week 12*Subgroup Interaction" VISITTYN1*trtn1 1 VISITTYN1*trtn1*wk12 1;
...
where trtn in (2,3);
class id avisitn SSNRISN1 ;
model chg=base trtn2 wk12 SSNRISN1 VISITTYN1 base*wk12 VISITTYN1*trtn2 trtn2*wk12 VISITTYN1*wk12 VISITTYN1*trtn2*wk12/ddfm=kr residual outp=residual;
repeated avisitn/type=un sub=id;/*unstructured covariance*/
estimate "1 ACT-PBO    Week 4" trtn2 1 VISITTYN1*trtn2 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 4" trtn2 1/cl alpha=0.05;
estimate "1 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1 VISITTYN1*trtn2 1 VISITTYN1*trtn2*wk12 1/cl alpha=0.05;
estimate "2 ACT-PBO  Week 12" trtn2 1 trtn2*wk12 1/cl alpha=0.05;
contrast "Treatment*Week 4*Subgroup Interaction" VISITTYN1*trtn2 1;
contrast "Treatment*Week 12*Subgroup Interaction" VISITTYN1*trtn2 1 VISITTYN1*trtn2*wk12 1;
...
set LSM_est1 LSM_est2;
     ...
     if label="1 ACT-PBO  Week 12" then VISITTYN=1;
else if label="2 ACT-PBO  Week 12" then VISITTYN=2;
...
JM_BLOCK1=strip(JM_BLOCK)||strip(put(VISITTYN,best.));
JM_BLOCK=JM_BLOCK1;
run;
...
%Macro stats_null(n=,SUBGRP_LAB=);
 ...
 length jm_aval_label SSNRIS _name_ _label_ trtn1 trtn2 $200 ;
  set JM_AVAL_TRANS&n(in=a) all_mixed_stat(in=b where=(VISITTYN=&n));
...
%mend;
%stats_null(n=1,SUBGRP_LAB=%str(VISIT : On-site ));

%stats_null(n=2,SUBGRP_LAB=%str(VISIT : Remote));
...
OPTIONS FORMCHAR='|_---|+|---+=|-/\<>*'; 
title%eval(&lasttitlenum.+1) j=l "Randomization stratum:  #BYVAL(VISITTYP)";

   %JM_AVAL_REPORT (JM_INDSN=jm_aval_allreport&myrep.,JM_BYVAR= VISITTYP, JM_BIGNDSN=Jm_bign1, 
...
%let dsname=T_9_02_01_03_06;
data tlfdata.&dsname;
length  TRTN1-TRTN3 $200.;
set final_subgrp_:;

keep JM_: TRTN: VISITTY:;
run;
