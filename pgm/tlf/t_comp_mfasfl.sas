...
%macro t_comp_mfasfl(trtvar=TRT01A,tab_page_size=12,
                   adsl_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y'),
				   adexsum_subset=%str(&TRTVAR.N in (1,2,3) and MFASFL='Y' and paramcd="COMPL" )
                 );
   ...
   *** Call JM_AVAL_COUNT macro once for each summary variable ***;                             
   %jm_aval_sum(JM_INDSN=target, jm_var=aval,jm_secondary_where=%str(paramcd="COMPL"), jm_bign=, jm_trtvarn=trtn,    JM_BLOCK=101,JM_SIGD=1 ,
   JM_AVAL_LABEL=%bquote(Compliance (%)) );

    %jm_aval_count(
      jm_indsn=target,jm_var=AVALCA1N,jm_secondary_where=%str(paramcd="COMPL"), jm_fmt=compln., 
      jm_bign=jm_bign1, jm_trtvarn=trtn,jm_block=102, jm_cntvar=usubjid,
      jm_aval_label=%bquote(Compliance Rate)      );
