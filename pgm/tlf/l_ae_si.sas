...
%let pgm = %scan(&sysin, -1, %(\/));
%let mypgmname = %scan(&pgm, 1, %str(.));

OPTIONS MPRINT NONUMBER;
*-----------------------------------------------------------------;
*TITLE AND FOOTNOTES
*-----------------------------------------------------------------;
%jm_tf (jm_infile=&tocpath., JM_PROG_NAME=&mypgmname.,  JM_PRODUCE_STATEMENTS=Y, JM_HEADER= &JM_HEADER_.);
%let output=&valoutnm;
...
%JM_PGBRK (JM_INDSN1=AE, JM_INDSN2=, JM_BREAKCNT=5, JM_CONTOPT=N, JM_GROUPOPT=N, JM_OUTDSN=JM_AVAL_ALLREPORT, JM_REPTYPE=Listing);
