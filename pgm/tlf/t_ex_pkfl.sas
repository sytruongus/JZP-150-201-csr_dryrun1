...
%macro t_ex_pkfl(trtvar=TRT01A,tab_page_size=12,
                   adsl_subset=%str(&TRTVAR.N in (1,2,3) and PKfl='Y'),
				   adexsum_subset=%str(&TRTVAR.N in (1,2,3) and PKFL='Y' )
                 );
   ...
   *** Call JM_AVAL_COUNT macro once for each summary variable ***;                             
   %jm_aval_sum(JM_INDSN=target, jm_var=aval,jm_secondary_where=%str(paramcd="EXPDAVG"), jm_bign=, jm_trtvarn=trtn,    JM_BLOCK=101,JM_SIGD=0 ,
   JM_AVAL_LABEL=%bquote(Average Total Daily Dose (mg/day)) );



  *** Call JM_AVAL_COUNT macro once for each summary variable ***;                             
   %jm_aval_sum(JM_INDSN=target, jm_var=aval,jm_secondary_where=%str(paramcd="EXDUR"), jm_bign=, jm_trtvarn=trtn,    JM_BLOCK=102,JM_SIGD=0 ,
